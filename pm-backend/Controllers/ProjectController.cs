...
namespace pm_backend.Controllers
{
    ...
    public class ProjectController : ControllerBase
    {
        ...
        [Authorize]
        [ProducesResponseType(typeof(ProjectIsDeletedRequest), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> UpdateIsDeleted(int id, ProjectIsDeletedRequest projectIsDeleted)
        {
            ...
            try
            {
                ...
                await _projectCommandService.UpdateIsDeletedAsync(id, projectIsDeleted.IsDeleted);
        ...
        [HttpDelete("{prjectId}/tasks/{id}")]
        [Authorize]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> DeleteTask(int prjectId, int id)
        {
            if (prjectId <= 0)
                return BadRequest("Invalid project id.");
            ...
            try
            {
                await _projectTaskCommandService.DeleteTaskAsync(id);
                return NoContent();
            }
            catch (KeyNotFoundException ex)
            {
